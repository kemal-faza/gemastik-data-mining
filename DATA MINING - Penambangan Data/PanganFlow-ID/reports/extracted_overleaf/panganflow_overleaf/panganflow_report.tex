...
Fokus penelitian ini adalah mengembangkan sistem \textit{data mining} PanganFlow.ID untuk memecahkan tiga rumusan masalah utama:
\begin{enumerate}
    \item provinsi mana yang mengalami tekanan pasokan dan gejolak harga pangan paling kritis,
    \item wilayah mana yang memiliki potensi surplus terbesar untuk menyokong kebutuhan daerah lain, serta
    \item pasangan asal-tujuan (\textit{flow}) antardaerah mana yang menjadi prioritas utama untuk intervensi redistribusi.
\end{enumerate}
Adapun batasan masalah dalam penelitian ini didefinisikan sebagai berikut: (1) komoditas dibatasi pada Beras Medium untuk menjaga konsistensi data produksi BPS dan harga eceran Bapanas; (2) neraca surplus-defisit dihitung berdasarkan estimasi kebutuhan tahunan kolektif (\textit{proxy} struktural), bukan mencerminkan stok fisik harian di gudang; dan (3) rekomendasi aliran redistribusi diposisikan sebagai acuan prioritas tinjauan kebijakan pemerintah, bukan sebagai perintah logistik operasional final.
...
\begin{table}[H]
  \footnotesize
  \caption{Status sumber data resmi.}
  \label{tab:source_status}
  \begin{tabular}{lrr}
    \toprule
    Sumber Data & Status & Baris \\
    \midrule
  ...
  \end{tabular}
  \\vspace{2pt}
  {\footnotesize \textit{Catatan:} Baris dengan nilai 0 menandakan endpoint PIHPS tidak mengembalikan data untuk parameter yang digunakan.}
\end{table}
...
Bobot pada persamaan (\ref{eq:priority}) diperoleh dari rata-rata \textit{feature importance} model \textit{Random Forest} yang telah dinormalisasi, dikombinasikan dengan \textit{expert calibration} untuk memastikan bahwa \textit{price\_gap} serta \textit{surplus/deficit} tetap menjadi kontributor dominan. Tanda negatif pada \textit{distance} merepresentasikan penalti logistik spasial.
...
Tabel \ref{tab:ablation} menyajikan hasil studi ablasi (\textit{ablation study}) terhadap kontribusi masing-masing kelompok fitur. Fitur neraca pangan (\textit{balance}) terbukti menjadi kontributor terbesar bagi performa model; ketiadaan fitur ini menurunkan NDCG@K dari 0,905 menjadi 0,880.
Menariknya, konfigurasi tanpa fitur spasial (NDCG@K=0,912) sedikit mengungguli konfigurasi semua fitur (NDCG@K=0,905). Fenomena ini disebabkan oleh redundansi informasi antara \textit{Haversine distance} dan variabel pengelompokan wilayah (\textit{region grouping}). Ketika kedua informasi ini hadir bersamaan, \textit{multicollinearity} menimbulkan \textit{noise} yang sedikit menurunkan kemampuan generalisasi model. Sebaliknya, fitur spasial memberikan kontribusi marginal terkecil karena efek kedekatan geografis sebagian besar telah direpresentasikan oleh pengelompokan wilayah (\textit{region grouping}).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.48\columnwidth]{model_comparison.png}
...
\begin{table}[H]
  ...
  \end{tabular}
\end{table}
Gambar \ref{fig:feature_importance} menampilkan peringkat fitur berdasarkan \textit{impurity-based importance}, sementara Gambar \ref{fig:shap_summary} menyajikan kontribusi fitur dari perspektif \textit{SHAP values} yang mempertimbangkan interaksi non-linear antarfitur.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.48\columnwidth]{feature_importance.png}
  \caption{Feature importance model prioritas.}
  \label{fig:feature_importance}
\end{figure}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.48\columnwidth]{shap_summary.png}
...
\begin{table}[H]
  ...
  \begin{tabular}{l l r}
    \toprule
    Peringkat & Aliran (Asal $\to$ Tujuan) & Skor Prioritas \\
    \midrule
    1 & Sulawesi Selatan $\to$ Papua Pegunungan & 100,0 \\
    2 & Jawa Timur $\to$ Papua Pegunungan & 95,54 \\
    3 & Jawa Tengah $\to$ Papua Pegunungan & 91,03 \\
    4 & Jawa Barat $\to$ Papua Pegunungan & 83,49 \\
    5 & Sumatera Selatan $\to$ Papua Pegunungan & 78,93 \\
    6 & Nusa Tenggara Barat $\to$ Papua Pegunungan & 76,72 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Peta centroid tekanan beras pada Gambar \ref{fig:pressure_map} mengonfirmasi konsentrasi wilayah rentan di Indonesia bagian timur, sedangkan Gambar \ref{fig:flow_network} merepresentasikan jaringan rekomendasi aliran antarprovinsi dengan ketebalan garis mencerminkan skor prioritas. Gambar \ref{fig:top_flows} menyajikan 10 pasangan aliran prioritas tertinggi yang dihasilkan sistem, dengan dominasi koridor Sulawesi dan Jawa menuju Papua.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.48\columnwidth]{pressure_centroid_map.png}
  \caption{Peta centroid tekanan beras nasional.}
  \label{fig:pressure_map}
\end{figure}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.48\columnwidth]{flow_network.png}
  \caption{Visualisasi jaringan rekomendasi aliran antarprovinsi.}
  \label{fig:flow_network}
\end{figure}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.48\columnwidth]{top_flow_recommendations.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.45\columnwidth]{dashboard_preview.png}
...
Pemetaan tipologi spasial wilayah pangan dilakukan menggunakan algoritme \textit{K-Means clustering} untuk mengelompokkan 38 provinsi menjadi 4 klaster berdasarkan skor harga dan proxy neraca beras nasional (Tabel \ref{tab:clusters}). Pemilihan \texttt{k=4} didasarkan pada analisis \textit{elbow method} terhadap inersia (\textit{within-cluster sum of squares}) dan \textit{Silhouette score}, di mana \texttt{k=4} memberikan keseimbangan optimal antara kohesi intra-klaster dan separasi antar-klaster. Hasil klasterisasi divisualisasikan dengan PCA pada Gambar \ref{fig:cluster_pca} yang memperlihatkan pemisahan klaster secara jelas.
...
\begin{table}[H]
  ...
  \begin{tabular}{l r r r}
    \toprule
    Label Klaster & Provinsi & Defisit & Surplus \\
    \midrule
    Defisit Kritis & 1 & 0,332 & 0,090 \\
    Defisit Moderat & 12 & 0,149 & 0,232 \\
    Surplus Moderat & 21 & 0,025 & 0,314 \\
    Surplus Melimpah & 4 & 0,003 & 0,879 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.48\columnwidth]{cluster_pca.png}
...
\begin{table}[H]
  ...
  \begin{tabular}{l r r r}
    \toprule
    Wilayah & Top-10 & Pangsa & Rerata Skor \\
    \midrule
...
\begin{table}[H]
  ...
  \begin{tabular}{l l p{2.0cm}}
    \toprule
    Provinsi Tujuan & Asal & Interpretasi Sinyal \\
    \midrule
    Papua Pegunungan & Sulawesi Selatan & Harga $>$ median; tren naik; defisit \\
    DKI Jakarta & Jawa Timur & Defisit \textit{proxy} neraca struktural \\
    Kalimantan Timur & Sulawesi Selatan & Harga $>$ median; defisit \textit{proxy} \\
    Papua Tengah & Sulawesi Selatan & Harga $>$ median; tren naik; defisit \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{thebibliography}{99}
  ...
  \bibitem{3} C. A. Bahri and K. D. Tania, ``Perbandingan Kinerja LSTM, Random Forest, dan SVR Berbasis Knowledge Discovery untuk Prediksi Harga Beras Sumatera Selatan,'' JURIKOM (Jurnal Riset Komputer), vol. 12, no. 5, pp. 721--732, 2025, doi: \href{https://doi.org/10.30865/jurikom.v12i5.9140}{10.30865/jurikom.v12i5.9140}.
  \bibitem{4} M. Ormanda and I. Ardiansah, ``Komparasi Model ARIMA, Regresi Linear, Random Forest, dan LSTM untuk Peramalan Harga Beras Jawa Barat,'' Jurnal Informatika Terpadu, vol. 12, no. 1, pp. 70--76, 2026, doi: \href{https://doi.org/10.54914/jit.v12i1.2789}{10.54914/jit.v12i1.2789}.
  \bibitem{5} D. Mufidah, N. Santoso, and M. A. Nafis, ``Rice Price Prediction in East Java Based on Weather Using Long Short-Term Memory (LSTM),'' Journal of Applied Informatics and Computing (JAIC), vol. 10, no. 2, pp. 1561--1567, 2026.
  ...
  \bibitem{7} J. I. Q. Adiba, J. Slamet, A. M. Teguh, A. F. Septiyanto, and R. Sarno, ``Advanced Deep Learning and Hybrid Models for Forecasting Rice Production in Indonesia,'' in 2024 11th International Conference on Information Technology, Computer, and Electrical Engineering (ICITACEE), IEEE, pp. 302--307, 2024, doi: \href{https://doi.org/10.1109/ICITACEE62763.2024.10762812}{10.1109/ICITACEE62763.2024.10762812}.
  ...
  \bibitem{9} ``Crop Redistribution Increases Regional Production While Reducing Resource Use,'' MDPI Agronomy, vol. 15, no. 9, p. 2148, 2025, doi: \href{https://doi.org/10.3390/agronomy15092148}{10.3390/agronomy15092148}.
