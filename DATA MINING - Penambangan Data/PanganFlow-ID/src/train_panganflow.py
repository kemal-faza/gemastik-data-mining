...
def shap_explainer(model: Pipeline, test: pd.DataFrame) -> str | None:
    """Generate SHAP summary plot using TreeExplainer. Returns path to saved plot or None."""
    if not HAS_SHAP:
        return None
    try:
        features = all_features()
        ...
        X_test = preprocessor.transform(test[features + CATEGORICAL_FEATURES])
        ...
        path = str(FIGURES / "shap_summary.png")
        plt.tight_layout()
        plt.savefig(path, dpi=220, bbox_inches="tight")
        plt.close()
        return path
    except Exception as exc:
        print(f"[SKIP] SHAP explainer failed: {exc}")
        return None
...
def error_analysis(scored_test: pd.DataFrame) -> pd.DataFrame:
    """Analyze prediction errors per region and province for the report."""
    scored_test = scored_test.copy()
    scored_test["error"] = scored_test["future_priority"] - scored_test["score_panganflow"]
    scored_test["abs_error"] = scored_test["error"].abs()
    scored_test["true_positive"] = (scored_test["future_label"] == 1) & (scored_test["score_panganflow"] >= scored_test["score_panganflow"].median())
    scored_test["false_positive"] = (scored_test["future_label"] == 0) & (scored_test["score_panganflow"] >= scored_test["score_panganflow"].median())
    scored_test["false_negative"] = (scored_test["future_label"] == 1) & (scored_test["score_panganflow"] < scored_test["score_panganflow"].median())

    # Per-region error summary
    region_error = scored_test.groupby("region", as_index=False).agg(
        n=("province", "count"),
    ...
    region_error.columns = ["region", "n", "n_provinces", "mae", "rmse", "bias", "fp_rate", "fn_rate"]
    region_error["region"] = region_error["region"].astype(str)

    # Per-province error summary (top-10 highest error)
    province_error = scored_test.groupby("province", as_index=False).agg(
        region=("region", "first"),
        n=("future_priority", "count"),
        mae=("abs_error", "mean"),
        rmse=("error", lambda s: float(np.sqrt(np.mean(s ** 2)))),
        bias=("error", "mean"),
        mean_actual=("future_priority", "mean"),
        mean_predicted=("score_panganflow", "mean"),
    )
    province_error = province_error.sort_values("mae", ascending=False).reset_index(drop=True)

    region_error.to_csv(TABLES / "error_analysis_region.csv", index=False)
    province_error.head(15).to_csv(TABLES / "error_analysis_province.csv", index=False)
    print(f"Error analysis: {len(region_error)} regions, {len(province_error)} provinces")
    print(f"  Mean MAE per region: {region_error['mae'].mean():.4f}")
    print(f"  Worst region: {region_error.loc[region_error['mae'].idxmax(), 'region']} (MAE={region_error['mae'].max():.4f})")
    print(f"  Worst province: {province_error.iloc[0]['province']} (MAE={province_error.iloc[0]['mae']:.4f})")
...
def train_pipeline() -> None:
    ...
    shap_path = shap_explainer(model, test)
    if shap_path:
    ...
    err_province = error_analysis(scored_test)
