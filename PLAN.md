...
| | #1 **DengueCast-X** | #2 **EduDrop-ID** | #3 **JudolFlow-X** |
|---|-----------------|-----------------|-----------------|
| Score | **91/100** | **87/100** | **84/100** |
| Pilar Kemandirian Bangsa | Kesehatan Publik | Pendidikan | Keamanan Digital |
| Mengapa | Multi-branch pipeline = winner pattern; highest impact | Zero GPU; data Dapodik; hierarchical model | NLP + Graph = core CS; KDD-style |
| Butuh | GPU, data DBD per kabupaten | CPU only (zero GPU) | GPU optional; scraping etis |
| Risiko | Data DBD availability | Novelty medium | Execution risk HIGH; data ethics |
| Skip Jika | Data DBD unavailable | Ingin deep learning / high novelty | Tidak siap handle ethical issues |
...
|------------|----------|-----------|--------|
| Ingin winner pattern terkuat | **DengueCast-X** | EduDrop-ID | Multi-branch TFT+GNN+CatBoost; PR-AUC metrics |
| Zero GPU, mau aman | **EduDrop-ID** | DengueCast-X | XGBoost + Mixed Effects; data Dapodik terstruktur |
...
5. [Topic Cards (3 Topik)](#5-topic-cards-3-topik)
    - [Rank 1: DengueCast-X](#rank-1-denguecast-x)
    - [Rank 2: EduDrop-ID](#rank-2-edudrop-id)
    - [Rank 3: JudolFlow-X](#rank-3-judolflow-x)
...
> _EduDrop-ID: Prediksi Risiko Putus Sekolah Berbasis Hierarchical Data Mining untuk Prioritas Intervensi Daerah_
...
[Full detail ->](#appendix-b-edudrop-id)
...
[Full detail ->](#appendix-c-judolflow-x)
...
| 2 | **EduDrop-ID** | **87** | Pendidikan | **Low** | **No** | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | **Low** |
| 3 | JudolFlow-X | 84 | Keamanan Digital | **High** | Opt | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | Moderate |
...
| **EduDrop-ID** | Risiko terendah; data Dapodik terstruktur; zero GPU | Novelty medium |
...
|----------|----------|-----------|--------|
| **Safe** (90% success) | EduDrop-ID | DengueCast-X | Data terstruktur; zero GPU; hierarchical model |
| **Balanced** (70% success) | DengueCast-X | EduDrop-ID | Multi-branch; winner pattern; highest impact |
...
|---|----------|--------|-------|
| 1 | Tim punya akses GPU untuk training? | DengueCast / JudolFlow | EduDrop |
| 2 | Prioritas novelty di atas segalanya? | JudolFlow | Go to Q3 |
| 3 | Ingin dampak sosial paling visible? | DengueCast | EduDrop |
